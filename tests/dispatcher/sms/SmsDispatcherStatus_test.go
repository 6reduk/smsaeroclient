...
func (s *SmsDispatcherStatusShould) TestSmsDispatcherStatus_ValidRequestData_ReturnSuccess() {
	s.clientMock.On(
		"SendRequestFor",
		mock.MatchedBy(testUtil.PassAnyObject),
		mock.MatchedBy(testUtil.PassAnyObject),
	...
	assert.EqualValues(s.T(), responseModelStub.GetSms(), smsMessageResult)
}
